  ...
  <main class="md:ml-60 max-h-screen md:overflow-auto px-6 pb-8">
    <div class="max-w-4xl mx-auto">
      <div class="bg-white rounded-3xl p-8 mb-5">
        ...
        <p class="text-xl">Hari : {{date("l")}}, {{date("H:i:s")}}</p>
        <p class="text-xl">Tanggal : {{date("Y-m-d")}}</p>
            ...
            @foreach ($siswas as $siswa)
                <div class="grid grid-cols-12 gap-4 mt-4 hover:bg-slate-100 py-2 px-2">
                    ...
                    <div class="col-span-7 flex justify-between items-center">
                        <input name="{{$siswa->id}}" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                        <input name="{{$siswa->id}}" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                        <input name="{{$siswa->id}}" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                        <input name="{{$siswa->id}}" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                    </div>
